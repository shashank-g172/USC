...
create table DocProfile (`user_id` mediumint(8) unsigned NOT NULL primary key, duser_name varchar(85) unique, doctor_fname varchar(85) not null, doctor_lname varchar(85) not null,  phone_no int(10), email varchar(20), gender enum('Male', 'Female') not null, license_no int(16) not null, expirydate date not null, spec_data varchar(100) not null, personal_desc varchar(64), certificate1 longblob, certificate2 longblob, certificate3 longblob, fellowship varchar(100), foreign key (`user_id`) references users(`user_id`) on update cascade on delete cascade);

--
-- Table structure for table scprofile
--
drop table if exists `scprofile`;
create table scprofile(`user_id` mediumint(8) unsigned NOT NULL primary key, SCuser_name varchar(85) unique, CenterName varchar(40)not null, SCaddress varchar(30)not null, city varchar(20) not null, state varchar(10) not null, zip int(5) not null, SCphone int(10) not null,email varchar(20) not null, staff_size int(5) not null, details varchar(64), poc_name varchar(100), poc_contact varchar(100), poc_designation varchar(100), insurance text, foreign key (`user_id`) references users(`user_id`) on update cascade on delete cascade);
