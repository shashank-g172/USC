...
class ScheduleUserRepository implements IScheduleUserRepository
{
	...
	public function GetUserPermissions($userId)
	{
		...
		$userCommand = new GetUserWithPermissionsCommand($userId);
		$roleCommand = new GetGroupIdCommand($userId);
		
		$reader = ServiceLocator::GetDatabase()->Query($userCommand);
		$resources = array();
		
		$reader1 = ServiceLocator::GetDatabase()->Query($roleCommand);

		$row1 = $reader1->GetRow();
		$groupID = $row1[ColumnNames::GROUP_ID];

		while ($row = $reader->GetRow())
		{	
			if ($row[ColumnNames::PERMISSION] == 2 && $groupID == 4)
			{
				$resources[] = new ScheduleResource($row[ColumnNames::RESOURCE_ID], $row[ColumnNames::RESOURCE_NAME]);
			}
			else if ($row1[ColumnNames::GROUP_ID] != 4)
			{
				$resources[] = new ScheduleResource($row[ColumnNames::RESOURCE_ID], $row[ColumnNames::RESOURCE_NAME]);
			}
