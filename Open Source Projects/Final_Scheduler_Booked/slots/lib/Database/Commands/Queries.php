...
class Queries
{
	...
	const ASSIGN_RESOURCE_PERMISSIONS =
	'INSERT INTO
				user_resource_permissions (user_id, resource_id, permission_id)
				VALUES(@userid, @resourceid, 2)';
	
	const IS_DOCTOR = 
	'select count(*) as count from docprofile where user_id = @userid';
	
	const IS_SC = 
	'select count(*) as count from scprofile where user_id = @userid';
	
	const GET_SCHEDULE_ADMIN = 
	' SELECT *
        FROM users
        WHERE user_id IN ( SELECT user_id from user_resource_permissions where resource_id = @resourceid and permission_id = 2 )';
	
	const GET_RESERVATION_USER =
	' SELECT *
        FROM dcprofile
...
const GET_RESERVATION_LIST_TEMPLATE =
	...
	const GET_USER_RESOURCE_WITH_PERMISSIONS =
	'SELECT
			urp.user_id, urp.permission_id, r.resource_id, r.name
		FROM
			user_resource_permissions urp, resources r
		WHERE
			urp.user_id = @userid AND r.resource_id = urp.resource_id';
