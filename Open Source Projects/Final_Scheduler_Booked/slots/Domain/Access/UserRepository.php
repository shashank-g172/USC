...
require_once(ROOT_DIR . 'Web/connection.php');
...
class UserRepository implements IUserRepository, IAccountActivationRepository
{
	...
	{
		$command = new GetScheduleAdminsCommand($resourceId);
		$reader = ServiceLocator::GetDatabase()->Query($command);
		$users = array();
...
class UserItemView
{
	...
	{
		$id = $user->Id;
		$command = new GetIsDoctorCommand($id);
		$reader = ServiceLocator::GetDatabase()->Query($command);
		$row = $reader->GetRow();
		$url = gethostname();
		if ($row['count'] > 0)
		{
			$link = $url."/../../../../slots/Web/docprofile.php?uid=$id";
		}
	
		$command = new GetIsSCCommand($id);
		$reader = ServiceLocator::GetDatabase()->Query($command);
		$row = $reader->GetRow();
	
		if ($row['count'] > 0)
		{
			$link = $url."/../../../../slots/Web/searchprofile.php?sc_id=$id";
		}
		$link = new Url($link);
