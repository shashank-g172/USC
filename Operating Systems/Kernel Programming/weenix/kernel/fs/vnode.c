...
find:
	...
	dbg_print("in vget: didnt find vnode in list, creating new one\n");
...
{
	dbg_print("INIT_SPECIAL_VNODE CALLED!!!\n");
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
	...
	dbg_print("in special_file_read\n");
	int retval;
	bytedev_t *bytedev;
	/*1.If the file is a block device then return -ENOTSUP*/
	if(S_ISBLK(file->vn_mode)) {
		return -ENOTSUP;
	} else {
		KASSERT(S_ISCHR(file->vn_mode)); 
		/*2.find the file's bytedev_t, and call read on it.*/
		bytedev = file->vn_cdev; /*should we use bytedev_lookup ? */
		retval = bytedev->cd_ops->read(bytedev, offset, buf, count);
		/*3.Return what read returns.*/
		dbg_print("special_file_read returning the no of bytes read as = %d\n", retval);
               	return retval;
        } 
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
	...
	dbg_print("in special_file_write\n");
	int retval;
	bytedev_t *bytedev;
	/*1.If the file is a block device then return -ENOTSUP*/
	if(S_ISBLK(file->vn_mode)) {
		...
		KASSERT(S_ISCHR(file->vn_mode)); 
		/*2.find the file's bytedev_t, and call write on it.*/
		bytedev = file->vn_cdev; /*should we use bytedev_lookup ? */
		retval = bytedev->cd_ops->write(bytedev, offset, buf, count);
		/*3.Return what write returns.*/
		dbg_print("special_file_write returning the no of bytes read as = %d\n", retval);
               	return retval;
        } 
}
