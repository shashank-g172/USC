...
ramfs_alloc_inode(fs_t *fs, int type, devid_t devid)
{
        ...
        for (i = 0; i < RAMFS_MAX_FILES; i++) {
				...
				dbg_print("ramfs_alloc_inode, putting devid in rf_mem\n");
...
ramfs_mount(struct fs *fs)
{
	...
	dbg_print("Calling vget for the root node\n");
        fs->fs_root = vget(fs, 0);
	dbg_print("returned from vget for the root node\n");
        return 0;
}
...
ramfs_lookup(vnode_t *dir, const char *name, size_t namelen, vnode_t **result)
{
        ...
        for (i = 0; i < RAMFS_MAX_DIRENT; i++, entry++) {
			...
			dbg_print("ramfs_lookup: calling vget as the dirent for %s was found\n", name);
...
ramfs_unlink(vnode_t *dir, const char *name, size_t namelen)
{
        ...
        off_t i;
        ramfs_dirent_t *entry;
	dbg_print("ramfs_unlink called with name = %s, name len = %d in dir = %d\n", name, namelen, dir->vn_vno);
