...
#define S4UDP181 "18602"
#define S4UDP191 "19602"
#define S4UDP201 "20602"
...
char *buffer = NULL;
   ...
   int string_size,read_size;
	...
	int vector[3];
	 int m=0;
   FILE *handler = fopen("Store-4.txt","r+");
...
// print information about this host:
	printf("Phase 1: store_4 has TCP port number %d", (int) ntohs(sin.sin_port));
...
perror("send");
printf("Phase 1: The outlet vector <%d,%d,%d> for store_4 has been sent to the central warehouse \n", vector[0],vector[1],vector[2]);
...
printf("Phase 2: Store_4 has UDP port number %s  ", S4UDP171);
for(i = 0; addr_list[i] != NULL; i++) 
...
//int yes=1;
if ((rv = getaddrinfo(NULL, S4UDP171, &hints, &servinfo)) != 0) {
fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
return 1;
}
...
//printf("Phase 2: Store_4 received the truck-vector <%d,%d,%d>from store 3\n", buffer1[0],buffer1[1],buffer1[2]);
...
printf("Phase 2: Store_4 has UDP port number %s and IP address ", S4UDP181);
addr_list = (struct in_addr **)he->h_addr_list;
for(i = 0; addr_list[i] != NULL; i++) 
		printf("%s \n", inet_ntoa(*addr_list[i]));
...
hints.ai_family = AF_UNSPEC;
hints.ai_socktype = SOCK_DGRAM;
if ((rv = getaddrinfo(hostname, S1UDP71, &hints, &servinfo)) != 0) {
fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
return 1;
}
...
hints.ai_flags = AI_PASSIVE; // use my IP
//int yes=1;
if ((rv = getaddrinfo(NULL, S4UDP191, &hints, &servinfo)) != 0) {
fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
return 1;
}
// loop through all the results and bind to the first we can
...
printf("Phase 2: Store_4 has UDP port number %s  ", S4UDP191);
for(i = 0; addr_list[i] != NULL; i++) 
		printf("and IP address %s \n", inet_ntoa(*addr_list[i]));
...
printf("Phase 2: Store_4 has UDP port number %s  ", S4UDP201);
for(i = 0; addr_list[i] != NULL; i++) 
		printf("and IP address %s \n", inet_ntoa(*addr_list[i]));
...
hints.ai_family = AF_UNSPEC;
hints.ai_socktype = SOCK_DGRAM;
if ((rv = getaddrinfo(hostname, WUDP321, &hints, &servinfo)) != 0) {
fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
return 1;
}
// loop through all the results and make a socket
